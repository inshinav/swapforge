...
## P10 Наглядная библиотека луков (запрос Alex 23.07.2026)

- [x] P10.1 Выбор лука в карусели = наглядная ГАЛЕРЕЯ превью-листов (варианты модели с
      role=model листом), не выпадашка; клик = выбор, подсветка+галочка.
- [x] P10.2 «+ Новый лук» инлайн: название + загрузка листа → addModelVariant + addModelRef
      (role=model) — переиспользует конструктор моделей, луки видны и в «Пресетах».
- [x] P10.3 LookStep переименован «Уточнить образ и пропсы» (осн. лук на шаге 1).
      Чистый фронтенд: БД/сервер/видео/пайплайн не тронуты (одежда идёт с character-sheet
      автоматом — доказано приёмкой). — check: typecheck+lint+test+build + браузер-смок.
