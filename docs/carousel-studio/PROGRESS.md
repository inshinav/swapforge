...
ПРОД-ПРИЁМКА 23.07.2026 (Claude, по «делай» Alex): мердж @9fcf6ac + деплой + флаг CAROUSEL_STUDIO=1/OWNER_ONLY=1 включён; живой прогон owner-ом: карусель 53e46531 «Утро, песок и слишком пушистый хвост» — 3/3 слайда done (MotoLola fox-образ консистентен через якорь, QC 8/7/6..8/8/7, 1 авто-ретрай), авто-подпись (hook про хвост, 13 тегов), ZIP 1.08MB валиден, admin-summary работает, ФАКТ $0.31. Найден+задеплоен хотфикс @837f0e5: QC без req.models уходил на дефолт-модель со snapshot-именем вне прайс-кэша → cost NULL (недосчёт settle). APIFY_TOKEN не задан — майнер ждёт токена Alex. ОСТАЛОСЬ Alex: нажать Start у @SwapForgeBot (TG-доставка), APIFY_TOKEN, снять OWNER_ONLY когда готов.
...
- 2026-07-23 — P8 (луки/пропсы/UGC-boost, запрос Alex) — (hash этого коммита) — carousel_refs (look|prop, upload+копия модельных vehicle/object), look_note; движки получают ЛУК+ПРОПСЫ (RU-заметки ок, слайд-промты EN через outfit/propNote), storyboard +propNote (default — старые json живы) и система про инста-ракурсы (mirror/arm-length/friend POV, не повторять камеру подряд); генерация: identity(≤2)+anchor+look(≤1)+props(≤2) с нумерацией; UGC v2 блоки (iPhone-look, анти-глянец, своя лестница обновлена), QC-критерий IG-нативности, зерно в финализации; UI секция «Лук и что в кадре» + поле пропса в раскадровке. 399 тестов, браузер-смок секции пройден.

- 2026-07-23 — P9 (автоподбор, запрос Alex) — (hash этого коммита) — одна кнопка: персона→темы (carousel_discover, микро-hold на подборку)→hashtag-скрейп→топ-авторы→profile-майнинг; гейт-ветка collectionId (заодно ЧИНИТ vision-майнинг не-owner), холды майнера скоупятся подборкой + reconcileMinerHolds; discover_run_id/stats.discovered = рестарт-безопасность; UI-блок с темами-чипами и ценами. Диагноз «не вижу луков»: прод раздаёт правильный бандл — секция только у НОВОЙ карусели (draft), добавлена подсказка в создание. 404 тестов, браузер-смок пройден.

- 2026-07-23 — P10 (наглядные луки, запрос Alex + 11 присланных листов MotoLola) — (hash этого коммита) — выбор лука в карусели теперь ГАЛЕРЕЯ превью-листов (варианты модели), клик=выбор, + инлайн «Новый лук» (название+фото→addModelVariant+addModelRef role=model, переиспользует конструктор — луки видны и в Пресетах); LookStep→«Уточнить образ и пропсы». Диагноз «не вижу луков»: раньше был выпадашкой «вариант», теперь наглядно. ЧИСТЫЙ ФРОНТЕНД (весь серверный API уже был: addModelVariant/addModelRef/modelFileUrl). Одежда с character-sheet идёт автоматом (identity-лист доминирует над текстовым outfit — доказано приёмкой). Браузер-смок: галерея+выбор+добавление ок, 404 тестов зелёные. Присланные листы Alex грузит кнопкой «Новый лук» (файлы у него локально — сам залить не могу).
